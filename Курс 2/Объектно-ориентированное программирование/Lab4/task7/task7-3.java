/* Поле first – координата x, second – y точки на плоскости, вещественные числа. 
	...
	Метод, по варианту задания в task4 для вспомогательного класса языка Java,
   	возвращающий целое или вещественное значение, дополнить методом,
   	возвращающий значение через вспомогательный класс. [см.5) лек. 4]. 
	В main продемонстрировать все оба метода.
 */
...
class arbeit{
	...
	public void sum_Of_Angles(Result anw)
    {
        double angle = one.degrees_radians() + two.degrees_radians();
        while(angle >= 2 * Math.PI)
            angle -= 2 * Math.PI;
